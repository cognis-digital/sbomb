...
How **sbomb** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `sbomb` lives in the **Malware / binary** cluster.
...
| from | relation | to |
|---|---|---|
| `sbomb` | JSON findings compose with | [`yaragen`](https://github.com/cognis-digital/yaragen), [`binhunt`](https://github.com/cognis-digital/binhunt), [`entropyscan`](https://github.com/cognis-digital/entropyscan) |
| `sbomb` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `sbomb` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `sbomb` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the malware / binary cluster.** `sbomb` output feeds the next tool:
```bash
sbomb ... --format json > out.json          # this tool's findings (see `sbomb --help`)
...
sbomb ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
sbomb ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
sbomb ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `sbomb`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `sbomb` slots
